...
Location of the robot can be defined at instant $k$ using state variables \eref{eq:xk localistion}, which include position in $x$ and $y$ coordinates and orientation $\theta$. %TODO Inconsistency with previous \Theta, but leave as is

\begin{IEEEeqnarray}{CCCCCCCCC} \label{eq:xk localistion} %TODO All labels must not have whitespaces and special symbols in them. Correct later
	x_k & = & 
	\begin{bmatrix}
		x_{[x], k} \\
		x_{[y], k} \\
...
The control input, or relative displacement, can be expressed as shown in \eref{eq: Uk + noise}.
...
The system can be expressed as shown \vpageref[below]{eq: xk + wk}. %TODO Latex might actually push it elsewhere, so I used vref
...
and is defined by equation \eref{eq:zk}.
...
In the proposed system, motion capture system available in the laboratory is used to localize Rollo. It acts as absolute sensor by providing directly the position in $x$ and $y$ coordinates and orientation $\theta$ of the robot. Thus, $z_k$ in this case is simply expressed as \eref{eq:zk 2}.
...
The final measurement model can be expressed as shown in \eref{eq:zk + noise2}.
\begin{equation} \label{eq: zk + noise} %TODO Shouldn't this be expressed in a analog way to x_k as g(x_k-1, u_k-1) + v_k? Just a formatting and consistency issue
z_k = x_k + v_k
\end{equation}
...
\begin{IEEEeqnarray}{CCCCCCCCC}\label{eq:KFxk + noise function} %TODO Optimally there would be references to all previous partial steps how these equations were derived, but this alone is very good without.
	\begin{bmatrix}
		x_{[x], k} \\
		x_{[y], k} \\
		x_{[\theta], k} \\
...
%\scalefig{Graphics/40-ros--nodes-diagram.pdf}{0.75}{Basic structure of developed ROS nodes.\label{f:40-ros--nodes-diagram}}
...
\indent\indent
EKF algorithm was first simulated in MATLAB using logs from Rollo's test runs.
...
\scalefigcrop{Graphics/Results/ekfresult1.pdf}{0.75}{Results of EKF simulation based on a log file.\label{f:20-matlab-ekf-1}}{20}{200}{0}{200}
...
Figure \ref{f:20-matlab-ekf-stability} shows the results of EKF update for longer iterations to verify the stability of EKF. If original EKF equations are directly used with the system, the state estimate and state covariance matrix starts to diverge after sometime, as discussed in section \nameref{s:30-kalman-filter--divergenceE_k|k}. This figure shows result of performing Cholesky decomposition for measurement update.
...
The complete software that received localization from the motion capture system, controlled Rollo, processed data to implement EKF and visualization  was developed in ROS framework.
Rollo was manually controlled using the keyboard and communication with Rollo was done at 10Hz. The measurement node was updated 25 Hz. Hence, the EKF update was also performed approximately at a rate of 25Hz.
...
We tested Rollo with different process covariance matrix, $Q$ and measurement covariance matrix, $R$ to analyze EKF. We define the two matrices here again. (For details, sections \nameref{s:20-Model--MeasurementModel}, \nameref{s:20-Model--SysModeNoisel} and \nameref{s:30-kalmanfilter--divergenceE_k|k}can be referred again.) 
...
Table \ref{t:02qandr} enlists the combinations of \textbf{$q$} and \textbf{$r$} that were tested.
...
%\end{enumerate}
	\begin{table}
		...
		\begin{tabularx}{0.8\textwidth}{@{}|Y|Y|@{}}
			\hline
			\textbf{$q$} & \textbf{$r$} \\ \hline %TODO Add symbols here, descriptions are welcome
			\hline
			0.1 & 0.1 \\ \hline
			0.1 & 1.0 \\ \hline
			0.1 & 10.0 \\ \hline
...
\scalefig{Graphics/50-simulation-and-testing--visualization-node.png}{0.75}{Real time visualization: localization of the robot based on EKF, odometry alone and the measurement.\label{f:50-simulation-and-testing--visualization-node}}
