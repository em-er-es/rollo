...
The equations \eref{e:SL=rtheta} and \eref{e:SR=rtheta} \vpageref[above]{eq:SL=rtheta} can be solved simultaneously to compute $r$ and $l$.
...
\begin{equation*}\label{e:finalorientationCW}
	\mathrm{final\ orienation = initial\ orientation - \Theta}\nonumber
\end{equation*}
...
\begin{equation*}\label{e:finalorientationSt}
	\mathrm{final\ orienation = initial\ orientation}
\end{equation*}
...
\begin{enumerate}
	\item Limited resolution during integration (time increments,
	measurement resolution).
	\item Unequal wheel diameter (deterministic)
	\item Variation in the contact point of the wheel
	(deterministic)
	\item Unequal floor contact and variable friction can lead to
...
\indent\indent
\subsection{System Model with noise}\label{s:20-Model--SysModel}
