...
% del_theta = (SL - SR) / L ; %u(2) L is wheelbase
% del_S = (SL + SR) / 2; % u(1)
% u = [del_S,  del_theta];
...
h_x = [x_cp(1); x_cp(2); x_cp(3)];                  % measurement equation, h(x_k)
z_estimate = h_x
...
%Instead of standard equations, using cholesky factorization for a stable
%covariance matrix
